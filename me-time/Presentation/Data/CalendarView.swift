...
struct CalendarView: View {
    ...
    @State private var clickedCurrentMonthDates: Date?  /// 사용자가 클릭한 날짜
    ...
    // MARK: - 연월 표시
    private func yearMonthView() -> some View {
            ...
            ForEach(-firstWeekday ..< daysInMonth + visibleDaysOfNextMonth, id: \.self) { index in
                Group {
                    if index > -1 && index < daysInMonth {
                        ...
                        // let morningPaper = morningPaperList.first(where: { $0.createAt.formattedCalendarDayDate == date.formattedCalendarDayDate })
...
private struct CellView: View {
    ...
    //     if let emotionString = Constant.TodayEmotion.AllEmotions(rawValue: emotion ?? "") {
    //         return emotionString.emotionEmoji
    //     } else {
    //         return ""
    //     }
    // }
    ...
    fileprivate var body: some View {
        VStack {
            ...
            // Text(Constant.TodayEmotion.AllEmotions(rawValue: emotion ?? "")?.emotionEmoji ?? "")
            // Text(emotionEmoji)
        }
        .frame(height: 60)
    }
